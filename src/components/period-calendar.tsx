// src/components/period-calendar.tsx
...
// Removed unused cycleData parameter
const generateCalendarMonth = (
  ...
  // Example/basic period data; adjust as needed or integrate real cycleData later
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    ...
    if (isPeriod) {
      ...
      if (periodDay <= 2) intensity = 3; // Heavy
      else if (periodDay <= 4) intensity = 2; // Medium
    ...
    days.push({
      ...
      isFertileWindow:
        i >= fertileWindowStart && i <= fertileWindowEnd && i !== ovulationDay,
...
export function PeriodCalendar() {
  ...
  // Removed unused setCycleData; if cycle data integration needed later, re-add and use
  ...
  const calendarData = generateCalendarMonth(
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-7 gap-1">
            {calendarData.days.map((day, index) => (
              <div
                key={index}
                className={cn("aspect-square p-1 relative", !day.isCurrentMonth && "opacity-40")}
              >
                <div
                  className={cn(
                ...
                >
                  <span
                    className={cn("text-sm font-medium", day.isToday && "text-rose-600 dark:text-rose-400")}
                  >
                    {day.date.getDate()}
                  </span>
                  ...
                  {day.isPeriod && (
                    <div className="flex mt-1 space-x-0.5">
                      {Array.from({ length: day.intensity || 0 }).map((_, i) => (
                        <div
                          key={i}
                          className="w-1 h-1 rounded-full bg-rose-500 dark:bg-rose-400"
                        />
                      ))}
                    </div>
                  )}
                  ...
                  {/* Ovulation indicator */}
                  {day.isOvulation && (
                    <div className="w-2 h-2 mt-1 rounded-full bg-blue-500 dark:bg-blue-400" />
                  )}

                  {/* Fertile window indicator */}
                  {day.isFertileWindow && (
                    <div className="w-1 h-1 mt-1 rounded-full bg-blue-400 dark:bg-blue-300" />
                  )}

                  {/* Predicted indicator */}
                  {day.isPredicted &&
                    (day.isPeriod || day.isOvulation || day.isFertileWindow) && (
          ...
          <div className="flex flex-wrap justify-center gap-4 pt-2">
            <Badge variant="outline" className="bg-background">
              Cycle Length: {cycleData.averageCycleLength} days
            </Badge>
            <Badge variant="outline" className="bg-background">
              Period Length: {cycleData.averagePeriodLength} days
            </Badge>
            {/* Example next period: calculate based on lastPeriodStart + averageCycleLength */}
            <Badge variant="outline" className="bg-background">
              Next Period:{" "}
              {formatMonthYear(
                new Date(
                  cycleData.lastPeriodStart.getFullYear(),
                  cycleData.lastPeriodStart.getMonth(),
                  cycleData.lastPeriodStart.getDate() + cycleData.averageCycleLength
                )
              )}
            </Badge>
          </div>
